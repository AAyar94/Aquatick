...
class ActivityLevelViewModel @Inject constructor(
    ...
    var activityLevelState by mutableStateOf<ActivityLevel>(ActivityLevel.Average)
        private set

    private val _uiEvent = Channel<UiEvent>()
    val uiEvent = _uiEvent.receiveAsFlow()

    fun activityLevelChange(activityLevel: ActivityLevel) {
        activityLevelState = activityLevel
    }

    fun onNextClick() {
        viewModelScope.launch {
            preferences.saveActivityLevel(activityLevelState)
